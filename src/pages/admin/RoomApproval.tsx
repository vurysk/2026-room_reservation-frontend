...
const RoomApproval: React.FC = () => {
  ...
  const handleAction = async (
    ...
    const confirmMsg =
      action === "Approved" ? "Terima pengajuan ini?" : "Tolak pengajuan ini?";
    if (window.confirm(confirmMsg)) {
      const res = await reservationService.updateReservationStatus(
        roomCode!,
        action,
      );
      if (res.success) {
  ...
  if (loading)
    return <div className="loading-text">Loading Pending Requests...</div>;
              ...
              <tr key={index}>
                ...
                <td className="detail-col">
                  <span
                    ...
                    onClick={() =>
                      navigate(`/reservation/detail/${item.roomCode}`)
                    }
                  >
                    Detail
                  </span>
                ...
                <td className="btns-col">
                  <div className="btn-group">
                    <button
                      ...
                      onClick={() => handleAction(item.roomCode!, "Rejected")}
                    >
                      Reject
                    </button>
                    <button
                      className="btn-accept"
                      onClick={() => handleAction(item.roomCode!, "Approved")}
                    >
                      Accept
                    </button>
                  </div>
